...
dati <- readxl::read_excel(".../Dati.xlsx", sheet = "DataSet")

library(pivottabler)
pt_sales <- PivotTable$new()
pt_sales$addData(dati)
...
# adding Month column
dati_sales <- cbind(rownames(pivot_sales), pivot_sales)
colnames(dati_sales)[1] <- "Month"
rownames(dati_sales) <- NULL

# adding "-01" to Month in order to convert the column as date 
dati_sales[, "Month"] <- paste(dati_sales[, "Month"], "-01", sep = "")
dati_sales[, "Month"] <- as.Date(dati_sales[, "Month"])

dati_sales <- as.data.frame(dati_sales)


library(forecast)
past_sales = dati_sales[1 : (nrow(dati_sales)),]
...
tspast_sales <- ts(past_sales[, col], start = c(min( as.integer( unique(format(dati_sales$Month, "%Y")) ) ), 1), end = c(max( as.integer( unique(format(dati_sales$Month, "%Y")) ) ), 12), frequency = 12) 

model_sales = ets(tspast_sales, allow.multiplicative.trend = F)
...
tspast_sales_tab <- ts(past_sales[, col], start = c(min( as.integer( unique(format(dati_sales$Month, "%Y")) ) ), 1), end = c(max( as.integer( unique(format(dati_sales$Month, "%Y")) ) ), 12), frequency = 12) 

model_sales_tab <- ets(tspast_sales_tab, allow.multiplicative.trend = F)
